...
class Product {
	...
	public function show_product_options() {
		...
		if ( is_object( $product ) && $product->get_id() > 0 && ! $product instanceof \WC_Product_Grouped ) {
			...
			?>
            <input type="hidden" id="zaddon_base_price" value="<?= esc_attr( $price ); ?>">
            <input type="hidden" id="zaddon_currency" value="<?= esc_attr( \get_woocommerce_currency() ); ?>">
            <input type="hidden" id="zaddon_locale" value="<?= esc_attr( get_locale() ); ?>">

            <div class="zaddon_data">
				<?php if ( get_customize_addon_option( 'zac_show_subtotal', true ) ) : ?>
                    <div class="zaddon_subtotal">
                        <h4><?php esc_html_e( 'Subtotal', 'product-add-ons-woocommerce' ); ?>:</h4>
				...
				<?php if ( get_customize_addon_option( 'zac_show_addons_total', true ) ) : ?>
                    <div class="zaddon_additional">
                        <h4><?= wp_kses_post( $add_ons_total_label ) ?>:</h4>
                        <p>+&nbsp;<span class="woocommerce-Price-amount amount"></span></p>
                    </div>
				<?php endif; ?>
				<?php if ( get_customize_addon_option( 'zac_show_total', true ) ) : ?>
                    <div class="zaddon_total">
                        <h4><?php esc_html_e( 'Total', 'product-add-ons-woocommerce' ); ?>:</h4>
                        <span class="woocommerce-Price-amount amount"></span>
                    </div>
				<?php endif; ?>
            </div>
	...
	public function add_cart_item_data( $cart_item_meta, $product_id, $variation_id, $post_data = null ) {
		...
		$zaddon = Utils::array_mapk( function ( $group_key, $group ) {
			return Utils::array_mapk( function ( $type_key, $type ) use ( $group_key ) {
				...
				list( $type, $should_return_current ) = apply_filters( 'zaddon_cart_item_addon_meta_before_default', array( $type, false ) );
									...
									function ( $total, $value ) use ( $typeAddon, $type, $price, $cart_item ) {
										...
										switch ( $type['type'] ) {
											...
											case 'radio':
												...
												break;
											case 'checkbox':
												$type_value = is_array( $typeAddon['value'] ) ? $typeAddon['value'] : array();
												$selected   = in_array( $value['id'] ?? null, $type_value );
												break;
											case 'text':
											default:
										...
										list( $addon_additional, $should_return_current ) = apply_filters( 'zaddon_calculate_cart_item_addon_additional', array( 0, false ), $typeAddon, $type, $price, $addon_value, $value['id'] );
										if ( $should_return_current ) {
	...
	public static function order_item_meta( $item_id, $item ) {
		...
		if ( $exist && isset( $item->legacy_values['_zaddon_values'] ) ) {
			header( 'typer: legacy' );
                ...
                jQuery(document).ready(function ($) {
                    ...
                    $("#product_zaddons").on("click", "#zaddon_submit", function (e) {
                        ...
                        var data = {
                            ...
                            "post_id": <?= wp_kses_post( isset( $post->ID ) ? $post->ID : 0 ); ?>
                        };
	...
	public function wc_tab_addons( $post ) {
        ...
        <div id="product_zaddons" class="panel woocommerce_options_panel">
            <div class="options_group" style="padding: 10px">
                <label><?php esc_html( _x( 'Name', 'Option name', 'product-add-ons-woocommerce' ) ); ?></label>
                <input
                        name="zaddon_group_name"
                        id="zaddon_group_name"
                        placeholder="<?php esc_html_e( 'Group name', 'product-add-ons-woocommerce' ); ?>"
                        value=""
                        style="width: 200px;"
                        type="text">
                <input
                        ...
                        value="<?php esc_html_e( 'Add Group', 'product-add-ons-woocommerce' ); ?>"
                        type="button">
            </div>
                                    ...
                                    <a href="<?= esc_attr(
	                                    add_query_arg( [
		                                    ...
		                                    'height'    => 340
																		...
																		); ?>" onclick="return false;" class="thickbox button">
										<?php esc_html_e( 'Edit', 'product-add-ons-woocommerce' ); ?>
                                    </a></td>
                            </tr>
						<?php } ?>
                        </tbody>
            ...
            <div class="options_group">
                <p class="form-field">
                    <label for="_zaddon_disable_global"><?php esc_html_e( 'Disable globals',
                    ...
                    <span class="description">
						<?php esc_html_e( 'Check this box if you want to disable global groups and use the above ones only!',
	...
	public function add_to_cart_validation( $status, $product_id ) {
		...
		if ( is_object( $product_item ) && $product_item->get_id() > 0 ) {
			...
			$product_id   = $is_variation ? sanitize_text_field( wp_unslash( $_POST['variation_id'] ) ) : $product_id;
			...
			foreach ( $groups as $group ) {
				foreach ( $group->types as $type ) {
					...
					$option_values = isset( $_POST['zaddon'][ $group->getID() ][ $type->getID() ] ) && is_array( $_POST['zaddon'][ $group->getID() ][ $type->getID() ] ) ?
						Utils::array_mapk(
							function ( $key, $val ) {
								switch ( $key ) {
									case 'value':
										return Utils::array_mapk(
											function ( $key, $val ) {
							...
							$_POST['zaddon'][ $group->getID() ][ $type->getID() ] // phpcs:ignore
						) : array();
	...
	public function ajax_save_group() {
		$name    = esc_sql( isset( $_POST['group_name'] ) ? sanitize_text_field( wp_unslash( $_POST['group_name'] ) ) : '' );
	...
	public function get_variation_section() {
		...
		$applied_ids  = isset( $_GET['applied_ids'] ) ? array_map(
			function ( $val ) {
				return sanitize_text_field( wp_unslash( $val ) );
			},
			$_GET['applied_ids'] // phpcs:ignore
		) : array();
