...
if ($type->status === 'disable') {
	...
	<div class="zaddon-type-container <?= $type->accordion === 'close' && $format !== 'select' ? 'zaddon_closed' : ''?>" data-id="<?= esc_attr( $type->getID() ) ?>">
    <h3>
        <?= wp_kses_post( $type->title ) ?>  <?= ! $is_select ? '<button class="zaddon-open" type="button"></button>' : ''?>
        <?php if ($type->tooltip_description && !$type->hide_description && $type->description) { ?>
            <span class="za-tooltip">
                <button class="za-tooltip__control" type="button" aria-label="<?php echo esc_attr( $type->description ) ?>"></button>
                <span class="za-tooltip__body"><?php echo esc_html( $type->description ) ?></span>
            </span>
        <?php } ?>
    </h3>
	<?php
    if (!$type->tooltip_description && !$type->hide_description && $type->description) {
        ...
        echo '<p class="' . esc_attr( join( ' ', (array) $classes ) ) . '">' . wp_kses_post( $type->description ) . '</p>';
    }
    ...
    <input type="hidden" name="<?= esc_attr( $name ) ?>[type]" value="<?= esc_attr( $format ) ?>">
