...
class SingleGroup {
	...
	protected function render() {
        ...
        <div class="wrap">
            <h1 class="nav-tab-wrapper woo-nav-tab-wrapper">
                <a href="<?= esc_url( Admin::getUrl( 'groups' ) ); ?>" class="nav-tab nav-tab-active">
	...
	protected function update() {
		...
		$group->types = array_map( function ( $typeData ) use ( $group ) {
			...
			$type->step                             = $typeData['step'] ?? 0;
			$type->title                            = $typeData['title'] ?? '';
			$type->required                         = boolval( $typeData['required'] ?? false );
			$type->hide_description                 = boolval( $typeData['hide_description'] ?? false );
			$type->display_description_on_expansion = boolval( $typeData['display_description_on_expansion'] ?? false );
			$type->description                      = $typeData['description'] ?? '';
			$type->tooltip_description              = boolval( $typeData['tooltip_description'] ?? false );
			...
			$type->values = array_map( function ( $valueData ) use ( $type ) {
				...
				$value->step                = $valueData['step'] ?? 0;
				$value->hide                = $valueData['hide'] ?? 0;
				$value->hide_description    = $valueData['hide_description'] ?? 0;
				$value->title               = $valueData['title'] ?? '';
				$value->checked             = boolval( $valueData['checked'] ?? false );
				...
				$value->tooltip_description = boolval( $valueData['tooltip_description'] ?? false );
